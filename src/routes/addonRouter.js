...
function replaceTemplatePlaceholders(template, content, meta, req, isFull) {
  ...
  return template
    ...
    .replace(/FORCE_REFRESH_URL/g, `?type=${encodeURIComponent(type)}&id=${encodeURIComponent(id)}&force=true`)
    .replace(/TOGGLE_URL/g, `${toggleTo}?type=${encodeURIComponent(type)}&id=${encodeURIComponent(id)}`)
    .replace(/TOGGLE_TEXT/g, toggleText);
}
...
async function handleQuickReviewPage(req, res) {
  try {
    ...
    // Build structured content using formatEnforcer
    const reviewMeta = {
