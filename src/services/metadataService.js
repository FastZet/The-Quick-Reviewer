...
// Fetches and normalizes metadata from external APIs: TMDB (primary), TVDB (new), OMDb (fallback).
'use strict';

const axios = require('axios');

const TMDB_API_KEY = process.env.TMDBAPIKEY || null;
const OMDB_API_KEY = process.env.OMDBAPIKEY || null;

const TVDB_API_KEY = process.env.TVDBAPIKEY || process.env.TVDB_API_KEY || null;
const TVDB_PIN = process.env.TVDBPIN || process.env.TVDB_PIN || null;
...
async function getTvdbToken() {
  ...
  const now = Date.now();
  if (tvdbToken && now < (tvdbTokenExpiry - 60_000)) return tvdbToken;
  try {
    ...
    tvdbToken = token;
    // TVDB tokens typically last 24h; refresh a bit earlier
...
async function resolveImdbToTvdbId(imdbId, type) {
  ...
  const url = `https://api4.thetvdb.com/v4/search/remoteids?imdbId=${encodeURIComponent(imdbId)}&type=${t}`;
...
async function fetchTvdbDetails(type, tvdbId) {
  ...
  const baseUrl = isSeries ? `https://api4.thetvdb.com/v4/series/${tvdbId}` : `https://api4.thetvdb.com/v4/movies/${tvdbId}`;
  try {
    ...
    // TVDB often returns an "image" field that is already a full URL to artworks.thetvdb.com
    const posterUrl = data.image || null;
    // Backdrop is not always present in the base object; keeping null unless artwork was fetched separately
    const backdropUrl = null;

    // Languages: try to derive a reasonable array of language names/codes
    // TVDB v4 provides "originalLanguage", and may have "languages" arrays on some objects
    const languages = [];
...
async function fetchMovieSeriesMetadata(type, imdbId) {
  ...
  // OMDb Fallback
  if (OMDB_API_KEY) {
    console.log('[OMDB] TMDB/TVDB failed or unavailable. Falling back to OMDb for', imdbId, '.');
    try {
      ...
      if (res.data && res.data.Response === 'True') {
        ...
        return {
          ...
          posterUrl,
          backdropUrl: null // OMDb does not provide backdrop
        };
      }
    } catch (error) {
...
// --- Episode fetch remains TMDB -> OMDb, unchanged (TVDB episode lookup is more complex) ---
async function fetchEpisodeMetadata(seriesImdbId, season, episode) {
  ...
  // OMDb Fallback for episode-level
  if (OMDB_API_KEY) {
