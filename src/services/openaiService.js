...
const MAX_RETRIES = 2;
const OPENAI_MODEL = process.env.AI_MODEL || 'gpt-4.1';
...
async function generateReview(prompt) {
  ...
  while (attempt < MAX_RETRIES) {
    attempt += 1;
    try {
      const api = await getOpenAIClient();

      const enhancedPrompt = [
        'You are a professional film critic.',
        'When reviewing films, prioritize accuracy and use your most recent training data to provide current information.',
        'IMPORTANT:',
        '- Use the most recent figures and ratings you have',
        '- If uncertain about specific current data, acknowledge the limitation',
        '- Keep the structure and formatting exactly as requested',
      ...
      const response = await api.chat.completions.create({
        model: OPENAI_MODEL,
        messages: [
          { role: 'system', content: 'You are a professional film critic. Focus on accuracy and acknowledge when current data may not be available. Follow formatting instructions exactly.' },
      ...
      if (DEBUG_RESPONSE) {
        ...
        try { console.log(JSON.stringify(response, null, 2)); } catch (e) { console.log('OpenAI Raw response stringify failed', e?.message || 'unknown error'); }
        console.log('OpenAI END RAW RESPONSE');
      }

      const text = response?.choices?.[0]?.message?.content;
      if (!text || !text.trim().length) throw new Error('Empty response from OpenAI');
      ...
      if (usage) {
        ...
        if (costs) {
          ...
          console.log('OpenAI Estimated cost', inputCost.toFixed(4), outputCost.toFixed(4));
        }
      }

      return text.trim();
    } catch (err) {
      console.error('OpenAI Error on attempt', attempt, 'message', err?.message, 'status', err?.status, 'code', err?.code, 'type', err?.type);
      ...
      console.warn('OpenAI retrying in', backoff, 'ms...');
      await delay(backoff);
    }
  }
